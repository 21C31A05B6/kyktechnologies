...
async function loadEmployees(){
  ...
  try{
    ...
    tbody.innerHTML='';
    if(!rows.length){tbody.innerHTML='<tr><td colspan="6">No employees found.</td></tr>';return;}
    rows.forEach(u=>{
      const tr=document.createElement('tr'); tr.innerHTML='<td></td><td></td><td></td><td></td><td></td><td></td>';
      const photoCell=tr.cells[0]; const img=document.createElement('img');
      const src=u.profilePhoto||'/kyk_logo.webp';
      img.src=src; img.alt=(u.name||'Employee')+' profile'; img.style.width='38px'; img.style.height='38px'; img.style.borderRadius='50%'; img.style.objectFit='cover';
      photoCell.appendChild(img);
      tr.cells[1].textContent=u.name||'—'; tr.cells[2].textContent=u.email||'—';
      tr.cells[3].textContent=(u.role||'').replace(/_/g,' '); tr.cells[4].textContent=u.department||'—';
      tr.cells[5].textContent=u.status||'active'; tbody.appendChild(tr);
    });
  }catch(e){console.error(e);}
}
...
on('profileForm','submit',async e=>{
  ...
  if($('profilePassword').value) formData.append('password',$('profilePassword').value);
  const fileInput=$('profilePhotoInput'); if(fileInput && fileInput.files && fileInput.files[0]) formData.append('profilePhoto', fileInput.files[0]);
  try{await fetch('/api/profile',{method:'PUT',headers:{Authorization:'Bearer '+gToken},body:formData}); $('profilePassword').value=''; $('profilePhotoInput').value=''; setFormMsg($('profileMsg'),'Profile saved.',true); loadMyProfile();}catch(err){setFormMsg($('profileMsg'),err.message,false);}
});
