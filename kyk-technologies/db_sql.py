...
def _ensure_profile_photo_columns():
    """Backfill profile_photo columns for existing databases created before this feature."""
    try:
        with _engine.begin() as conn:
            inspector = inspect(conn)
            for table_name, columns in {"users": ["profile_photo"], "admins": ["profile_photo"], "employees": ["profile_photo"]}.items():
                ...
                for column_name in columns:
                    ...
                    if conn.dialect.name == "sqlite":
                        ...
                        conn.execute(text(f'ALTER TABLE "{table_name}" ADD COLUMN IF NOT EXISTS "{column_name}" VARCHAR(255)'))
    except Exception as e:
        print(f"Warning: Failed to backfill profile photo columns: {e}")
        ...
        with _Session() as session:
            ...
            if not user:
                ...
                if "profilePhoto" in body:
                    user.profile_photo = body.get("profilePhoto")
                if pw_hash:
            ...
            if role == "super_admin":
                ...
                if not adm:
                    session.add(Admin(user_id=user.id, name=name, email=email, is_super_admin=True, department="Executive", profile_photo=body.get("profilePhoto")))
                else:
                    adm.name = name
                    adm.email = email
                    if "profilePhoto" in body:
                        adm.profile_photo = body.get("profilePhoto")
            elif role == "employee":
                ...
                if not emp:
                    ...
                    if "profilePhoto" in body:
                        emp.profile_photo = body.get("profilePhoto")
                    if "status" in body:
